...
pub fn make_stream<F>(system: &Arc<System>, perform: F) -> anyhow::Result<cpal::Stream>
    where F: FnMut(&mut [f32], &cpal::OutputCallbackInfo) + std::marker::Send + 'static,
{
    ...
    let config = get_config(system, &device)
        .ok_or_else(|| anyhow::Error::msg(
            format!("No supported stream configuration for {}Hz sample rate and {} bufsize.",
                    system.samplerate(),
                    system.bufsize())
        ))?;
